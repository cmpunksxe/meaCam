...
//  Kamera
//
//  Created by XOR Co. Ltd. on 2018/07/04.
//  Copyright © 2018 priorIT. All rights reserved.
//
...
extension PHPhotoLibrary {
    ...
    func saveVideo(videoURL:URL, albumName:String, completion:((PHAsset?)->())? = nil) {
        func save2() {
            if let album = PHPhotoLibrary.shared().findAlbum(albumName: albumName) {
                PHPhotoLibrary.shared().saveVideo2(videoURL: videoURL, album: album, completion: completion)
            } else {
                PHPhotoLibrary.shared().createAlbum(albumName: albumName, completion: { (collection) in
                    if let collection = collection {
                        PHPhotoLibrary.shared().saveVideo2(videoURL: videoURL, album: collection, completion: completion)
                    } else {
                        completion?(nil)
                    }
                })
    ...
    fileprivate func saveVideo2(videoURL: URL, album: PHAssetCollection, completion:((PHAsset?)->())? = nil) {
        var placeholder: PHObjectPlaceholder?
        PHPhotoLibrary.shared().performChanges({
